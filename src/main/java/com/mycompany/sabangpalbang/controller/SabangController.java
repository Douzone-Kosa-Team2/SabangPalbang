...
public class SabangController {
	...
	public String inquirylist(int sid, String pageNo, Model model, HttpSession session) {
		...
		if (pageNo == null) {
			...
			Pager pager = (Pager) session.getAttribute("inquiry_pager");
			if (pager != null) {
				intPageNo = pager.getPageNo();
			}
		} else {
		...
		session.setAttribute("inquiry_pager", pager);
	...
	public String pop_ask_content(String inquiry_id, Authentication auth, Model model) {
		...
		/*
		 * Sabang sabang = sabangService.getSabang(Integer.parseInt(sid)); String
		 * user_email = auth.getName();
		 * 
		 * Member member = memberService.showMember(user_email);
		 * 
		 * model.addAttribute("sabang",sabang); model.addAttribute("member",member);
		 * 
		 */

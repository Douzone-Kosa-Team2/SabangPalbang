			...
			<table style="width:100%;">
				<tr>
					...
					<td colspan="5" class="text-center"><a
						class="btn btn-outline-primary btn-sm" href="palbang_main?pageNo=1">처음</a>
	
						<c:if test="${pager.groupNo>1}">
							<a class="btn btn-outline-info btn-sm"
								href="palbang_main?pageNo=${pager.startPageNo-1}">이전</a>
						</c:if> <c:forEach var="i" begin="${pager.startPageNo}"
							end="${pager.endPageNo}">
	
							<c:if test="${pager.pageNo!=i}">
								<a class="btn btn-outline-success btn-sm"
									href="palbang_main?pageNo=${i}">${i}</a>
							</c:if>
							<c:if test="${pager.pageNo==i}">
							...
							<a class="btn btn-outline-info btn-sm"
								href="palbang_main?pageNo=${pager.endPageNo+1}">다음</a>
						</c:if> <a class="btn btn-outline-primary btn-sm"
						href="palbang_main?pageNo=${pager.totalPageNo}">맨끝</a></td>
				</tr>
			</table>
		</div>
	</div>
